...
struct TransactionBarChartView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, ChartViewDelegate {
        ...
        func chartValueSelected(_ chartView: ChartViewBase, entry: ChartDataEntry, highlight: Highlight) {
            let month = Transaction.allTransactions[Int(entry.x)].month
    ...
    func formatXAxis(xAxis: XAxis) {
        xAxis.valueFormatter = IndexAxisValueFormatter(values: Transaction.allTransactions.map { value in String(value.month) })
...
struct TransactionBarChartView_Previews: PreviewProvider {
    static var previews: some View {
        TransactionBarChartView(entries: [BarChartDataEntry(x: 100, y: 200), BarChartDataEntry(x: 100, y: 300)],
